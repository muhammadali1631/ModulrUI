...
import { LiveComponentPreview } from "../components/LiveComponent";
...
const Generate = () => {
  ...
  const handleGenerate = async () => {
    ...
    try {
      const { data } = await axios.post(
        ServelUrl + "/api/component/generate",
        { prompt },
  ...
  const handleSave = async () =>{
    ...
    setSaving(true);
    try {
      const res = await axios.post(ServelUrl + '/api/component/save', {
  ...
  const handlePublished = async ()=>{
    ...
    setPublishing(true);
    try {
      await axios.post(ServelUrl + "/api/component/publish",{
        componentId:savedComponentId}, {withCredentials: true})
      setPublished(true)
      showToast("Published to NPM successfull!", "success")
      setPublishing(false)
    } catch (error) {
      console.log(error)
      ...
      <div className="relative z-10 max-w-5xl mx-auto px-4 py-12">
          ...
          >
            ...
            >
              <FiZap
                size={13}
                style={{
                  color: lowCredits ? "#f87171" : "#818cf8",
                }}
              />
        ...
        {lowCredits && (
          ...
          >
            ...
            <button
              onClick={() => navigate("/pricing")}
              className="ml-auto flex items-center gap-1.5 px-3 py-1.5 rounded-lg texxt-xs font-semibold transition-all cursor-pointer border-none whitespace-nowrap"
        ...
        >
          <div className="flex items-start gap-3 p-4">
            <FiZap size={20} className="text-indigo-400 mt-1 shrink-0" />
            <textarea
              ...
              disabled={lowCredits}
              rows={3}
              className="w-full bg-transparent text-white placeholder:text-white/20 text-[15px] resize-none outline-none leading-relaxed disabled:cursor-not-allowed"
            />
          </div>
          ...
          <div className="flex items-center justify-between px-4 pb-3">
            ...
            <motion.button
              ...
              whileTap={{ scale: 0.97 }}
              disabled={lowCredits || generating || !prompt.trim()}
        ...
        <AnimatePresence>
          {generated && (
            ...
            >
                ...
                >
                  {["preview", "code"].map((tab) => (
                    <button
                      ...
                      className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium transition-all capatalized"
                      style={{
                        background:
                          activeTab === tab
                            ? "rgba(99,102,241,0.5)"
              ...
              <div className="flex items-center gap-3 px-5 pb-5 pt-5 flex-wrap">
                {userRole === "admin" && (
                  <>
                    <motion.button
                      onClick={handleSave}
