...
function AddComponentForm() {
  ...
  const handleSave = async () => {
    if(!name.trim() || !code.trim()){
      showToast("Component name and code is required.", "error")
      return
    }
    setSaving(true);
    try {
      const res = await axios.post(
        ServelUrl + "/api/component/save",
        {
          name,
          code,
          props,
  ...
  const handlePublished = async () => {
    ...
    try {
      await axios.post(
        ServelUrl + "/api/component/publish",
        {
          componentId: savedId,
        },
        { withCredentials: true },
    ...
    return (
      <>
        <div className="flex items-center gap-2.5 px-5 py-5 border-b border-white/[0.06]">
          ...
          <div>
            <span className="font-bold text-base block">ModulrUI</span>
        ...
        <div className="sticky top-0 z-10 px-4 sm:px-6 lg:px-8 py-3.5 sm:py-4 bg-[#030b0d]/90 backdrop-blur-md border-b border-white/[0.05] flex items-center justify-between gap-2">
          <div className="flex items-center gap-3 min-w-0">
            <button
              ...
              className="md:hidden bg-transparent border-none cursor-pointer p-1.5 rounded-lg text-white/50 hover:bg-white/[0.05] hover:text-white/80 transition-all flex-shrink-0"
            >
              <TbMenu2 size={20} />
            </button>
          ...
          >
            <TbPlus size={14} />
            <span className="hidden md:inline">AI Component</span>
          </motion.button>
        </div>
                ...
                {stats.map(({ label, value, icon: Icon, color }, i) => (
                  ...
                  >
                    <div className="mb-2.5 sm:mb-3">
                      <div
                        className="w-8 h-8 sm:w-9 rounded-xl flex items-center justify-center"
                        style={{
                          background: `${color}15`,
