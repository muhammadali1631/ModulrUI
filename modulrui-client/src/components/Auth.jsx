...
function Auth({ onClose }) {
  ...
  const googleAuth = async () => {
    try {
      ...
      const result = await fetch(ServelUrl + "/api/auth/google",{
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({name, email}),
        credentials: 'include' // Must stringify the body
      })
    ...
    <AnimatePresence>
      ...
      >
        ...
        >
          ...
          <button
            onClick={onClose}
            className="absolute top-3 right-3 z-20 w-8 h-8 rounded-lg bg-white/5
          hover:bg-white/20 border border-white/10 flex items-center justify-center text-white/50
          ...
          <div className="sm:w-[52%] bg-gradient-to-br from-[#03181c] to-[#041e24] p-6 sm:p-10 relative overflow-hidden">
            ...
            bg-[radial-gradient(circle_at_center,rgba(59,232,255,0.08)_0%,transparent_70%)] 
            pointer-events-none"
            />
            ...
            <div className="flex sm:flex-col gap-2 sm:gap-1 overflow-x-auto sm:overflow-x-visible pb-2 sm:pb-0 -mx-1 px-1">
              {steps.map((item, i) => (
                ...
                >
                  <div
                    className={`min-w-[28px] h-7 rounded-lg flex items-center justify-center border transition-all duration-300 ${active === i ? "bg-gradient-to-br from-[#3be8ff] to-[#0ab8d6] border-transparent" : "bg-[#3be8ff]/[0.08] border-[#3ef8ff]/20"}`}
                  >
                    <item.icon
                      size={13}
                  ...
                  <div>
                    ...
                    <div
                      className={`overflow-hidden transition-all duration-500 ${active === i ? "max-h-8 opactity-100 mt-0.5" : " opacity-0"}`}
                    >
          ...
          >
            ...
            bg-[linear-gradient(rgba(59,232,255,0.025)_1px,transparent_1px),linear-gradient(90deg,rgba(59,232,255,0.025)_1px,transparent_1px)] 
            bg-[size:32px_32px]"
            />
            ...
            <div className="relative z-10 w-full max-w-[280px] sm:max-w-[260px] text-center mx-auto">
              ...
              transition={{duration:3, repeat:Infinity, ease:easeInOut}}
               className="w-12 h-12 sm:h-14 sm:w-14 rounded-2xl mx-auto mb-5 sm:mb-6 bg-gradient-to-br from-[#3be8ff]/15 to-[#040f12] border border-[#3be8ff]/20 flex items-center justify-center">
                <SiValorant size={22} color="#3be8ff"/>
              </motion.div>

              <h3 className="text-xl font-bold text-[#e4f6f8] tracking-tight mb-2" style={{ fontFamily: "'Syne', sans-serif" }}>Welcome</h3>
              <p className="text-[13px] text-[#96bec8]/55 leading-relaxed mb-6 sm:mb-7">Sign in to generate AI-powered UI components in seconds</p>
              ...
              hover:shadow-[0_12px_40px_rgba(59,232,255,0.2)] 
              transition-shadow">
                <FcGoogle size={19}/> Continue with Google 
              </motion.button>
                <p  className="text-[11px] text-[#64919b]/45 mt-4 sm:mt-5">
                No account needed for npm.{" "}<span onClick={onClose} className="text-[#3be8ff]/50 border-b border-[#3be8ff]/20 cursor-pointer hover:text-[#3be8ff]/80 transition-colors">View Docs →</span>
                </p>
